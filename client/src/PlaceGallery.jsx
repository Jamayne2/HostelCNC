      ...
      <div className="absolute inset-0 bg-black text-white min-h-screen">
        <div className="bg-black p-8 grid gap-4">
          ...
          {place?.photos?.length > 0 && place.photos.map(photo => (
            <div>
              <Image className="aspect-square cursor-pointer object-cover w-full p-4" src={photo} alt=""/>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid gap-2 grid-cols-[2fr_1fr] rounded-3xl overflow-hidden">
        ...
        <div className="grid">
          ...
          <div className="overflow-hidden">
            {place.photos?.[2] && (
              <Image onClick={() => setShowAllPhotos(true)} className="aspect-square cursor-pointer object-cover relative top-2 w-full h-full" src={place.photos[2]} alt=""/>
            )}
          </div>
        </div>
      </div>
